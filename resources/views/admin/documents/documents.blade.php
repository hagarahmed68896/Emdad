...
@section('page_title', __('messages.page_title'))

@section('content')

    <div class="p-6 overflow-y-auto">
        <p class="text-[32px] font-bold">{{__('messages.page_title')}}</p>
        <div>
                    ...
                    <div x-data="{ showDeleteModal: false }" @open-bulk-delete-modal.window="showDeleteModal = true">
                        <div x-show="showDeleteModal" x-cloak x-transition.opacity
                            ...
                            <div @click.away="showDeleteModal = false"
                                ...
                                <h2 class="text-lg font-bold mb-4">{{__('messages.bulk_delete_confirm_title')}}</h2>
                                <p class="text-gray-600 mb-6">
{{__('messages.bulk_delete_confirm_text')}}                                    <span x-text="selectedDocuments.length"></span>
                                </p>
                    ...
                    <form action="{{ route('admin.documents.index') }}" method="GET"
                        ...
                        <div x-data="{
                            ...
                            <div x-show="open" @click.away="open = false" x-transition.opacity x-cloak
                                ...
                                <!-- الترتيب حسب -->
                                <h3 class="font-bold text-gray-700 mb-2 rtl:text-right"> {{__('messages.filter_sort_by')}}</h3>
                                ...
                                <!-- نوع الوثيقة -->
                                <h3 class="font-bold text-gray-700 mb-2 rtl:text-right">{{__('messages.document_type')}}</h3>
                                <ul class="space-y-1 mb-4">
                                    <li>
                                        <label class="flex items-center cursor-pointer">
                                    ...
                                    <li>
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="document_name" value="Commercial Registration"
                                                ...
                                                class="shrink-0 rtl:ml-3 ltr:mr-3 w-5 h-5 border-[#185D31] appearance-none rounded-full border-2 checked:bg-[#185D31] checked:border-[#185D31]">
                                            <span class="text-gray-700">{{__('messages.commercial_registration')}}  </span>
                                        </label>
                                    </li>
                                    <li>
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="document_name" value="National ID"
                                                x-model="selectedType"
                                                class="shrink-0 rtl:ml-3 ltr:mr-3 w-5 h-5 border-[#185D31] appearance-none rounded-full border-2 checked:bg-[#185D31] checked:border-[#185D31]">
                                            <span class="text-gray-700"> {{__('messages.national_id')}}  </span>
                                        </label>
                                    </li>
                                    <li>
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="document_name" value="Tax Certificate"
                                                x-model="selectedType"
                                                class="shrink-0 rtl:ml-3 ltr:mr-3 w-5 h-5 border-[#185D31] appearance-none rounded-full border-2 checked:bg-[#185D31] checked:border-[#185D31]">
                                            <span class="text-gray-700"> {{__('messages.tax_certificate')}}</span>
                                        </label>
                                    </li>
                                    <li>
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="document_name" value="IBAN"
                                                x-model="selectedType"
                                                class="shrink-0 rtl:ml-3 ltr:mr-3 w-5 h-5 border-[#185D31] appearance-none rounded-full border-2 checked:bg-[#185D31] checked:border-[#185D31]">
                                            <span class="text-gray-700">{{__('messages.iban')}}</span>
                                        </label>
                                    </li>
                                    <li>
                                        <label class="flex items-center cursor-pointer">
                                            <input type="radio" name="document_name" value="National Address"
                                                x-model="selectedType"
                                                class="shrink-0 rtl:ml-3 ltr:mr-3 w-5 h-5 border-[#185D31] appearance-none rounded-full border-2 checked:bg-[#185D31] checked:border-[#185D31]">
                                            <span class="text-gray-700"> {{__('messages.national_address')}}</span>
                                        </label>
                                    </li>
                                </ul>
                        ...
                        <div class="relative w-full md:w-auto flex-1">
                            ...
                            <input type="text" name="search" value="{{ $search }}"
                                placeholder="{{__('messages.search_supplier_name')}}"
                        ...
                        <div x-data="{ open: false }" class="relative inline-block text-left">
                            @php
                                ...
                                $statusName = match ($status) {
                                    'approved' => __('messages.verified_documents'),
                                    'pending' => __('messages.documents_under_review'),
                                    'expired' => __('messages.expired_documents'),
                                    'rejected' => __('messages.rejected_documents'),
                                    default => __('messages.all_documents'),
                                };
                            @endphp
...
{{ __('messages.verified_documents')}}                                </a>
...
{{__('messages.documents_under_review')}}                                </a>
...
{{__('messages.expired_documents')}}                                </a>
...
{{__('messages.rejected_documents')}}                                </a>
                            </div>
                        </div>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse ($documents as $document)
                <tr>
                    ...
                    <td class="px-6 py-4 text-right">
                        @php
                            $documentName = match ($document->document_name) {
                                'National ID' => __('messages.national_id'),
                                'Tax Certificate' => __('messages.tax_certificate'),
                                'IBAN' => __('messages.iban'),
                                'National Address' => __('messages.national_address'),
                                'Commercial Registration' => __('messages.commercial_registration'),
                                default => '-',
                            };
                        @endphp
                        {{ $documentName }}
                        ...
                        @php
                            ...
                            switch ($document->status) {
                                case 'approved':
                                    ...
                                    $statusText = __('messages.verified');
                                    break;
                                case 'pending':
                                    $statusClass = 'bg-gray-100 text-gray-800';
                                    $statusText = __('messages.under_review');
                                    break;
                                case 'expired':
...
<a href="{{ route('documents.show', ['document' => $document->id]) }}" target="_blank">
    <i class="fas fa-eye"></i>
</a>
@endif
