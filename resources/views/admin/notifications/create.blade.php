...
<div class="p-6 bg-gray-50 min-h-screen">
    <div class="bg-white shadow rounded-xl p-6">
        <h2 class="text-2xl font-bold mb-6 text-gray-800">{{ __('messages.add_notification') }}</h2>
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="notification_type" class="w-full border rounded-lg p-2">
                        <option value="alert" {{ old('notification_type') == 'alert' ? 'selected' : '' }}>
                            {{ __('messages.alert') }}
                        </option>
                        <option value="offer" {{ old('notification_type') == 'offer' ? 'selected' : '' }}>
                            {{ __('messages.offer') }}
                        </option>
                        <option value="info" {{ old('notification_type') == 'info' ? 'selected' : '' }}>
                            {{ __('messages.info') }}
                        </option>
                    </select>
                </div>
