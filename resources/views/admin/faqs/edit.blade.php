...
@section('page_title', __('messages.edit_faq'))

@section('content')
<div class="p-6 bg-gray-50 min-h-screen">
    <h2 class="text-[28px] font-bold mb-6 text-gray-800">{{ __('messages.edit_faq') }}</h2>
    ...
    <div class="bg-white p-6 rounded-lg shadow">
        <form action="{{ route('admin.faqs.update', $faq->id) }}" method="POST" class="space-y-4">
            ...
            <!-- الفئة المستهدفة -->
            <div>
                <label class="block text-gray-700 font-bold mb-2">{{ __('messages.user_type') }}</label>
                <select name="user_type"
                        class="w-full border rounded-md p-2 shadow-sm focus:ring-green-600 focus:border-green-600"
                        required>
                    <option value="customer" {{ old('user_type', $faq->user_type) == 'customer' ? 'selected' : '' }}>
                        {{ __('messages.customer') }}
                    </option>
                    <option value="supplier" {{ old('user_type', $faq->user_type) == 'supplier' ? 'selected' : '' }}>
            ...
            <div class="flex gap-3">
                <button type="submit" class="px-4 py-2 bg-[#185D31] text-white rounded-md">{{ __('messages.update') }}</button>
                <a href="{{ route('admin.faqs.index') }}" class="px-4 py-2 bg-gray-400 text-white rounded-md">{{ __('messages.cancel') }}</a>
            </div>
        </form>
    </div>
</div>
