...
<div class="mx-[64px] mb-4 bg-white rounded-xl mt-4 p-6">
    ...
    <h1 class="text-3xl font-bold mb-6">{{ __('messages.add_offer') }}</h1>
    {{-- ✅ Success Message Container --}}
    <div id="success-message" class="hidden p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg" role="alert">
        <span class="font-medium"></span>
    </div>
    <form id="create-offer-form" enctype="multipart/form-data" class="space-y-6"
...
>
        ...
        {{-- Offer Name and Product Name --}}
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block font-bold mb-1">{{ __('messages.offer_name') }}</label>
                <input type="text" name="name" value="{{ old('name') }}" placeholder="{{ __('messages.enter_offer_name') }}" class="border p-2 w-full rounded-xl">
                @error('name') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
            </div>
            
            {{-- Product Selection Dropdown --}}
            <div class="relative" x-data="{ open: false, selectedProduct: null }" @click.away="open = false">
        ...
        {{-- Offer Period and Discount Percentage --}}
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block font-bold mb-1">{{ __('messages.offer_start') }}</label>
                <input type="date" name="offer_start" value="{{ old('offer_start') }}" class="border p-2 w-full rounded-xl">
            ...
            <div>
                ...
                <input type="date" name="offer_end" value="{{ old('offer_end') }}" class="border p-2 w-full rounded-xl">
            ...
            <div>
                ...
                <input type="number" name="discount_percent" value="{{ old('discount_percent') }}" class="border p-2 w-full rounded-xl" placeholder="{{ __('messages.enter_discount_percentage') }}">
        ...
        <div>
            ...
            <textarea name="description" rows="4" placeholder="{{ __('messages.enter_offer_description') }}" class="border p-2 w-full rounded-xl">{{ old('description') }}</textarea>
        ...
        <div>
            ...
            <select name="product_status" class="border p-2 w-full rounded-xl">
                ...
                <option value="ready_for_delivery" {{ old('product_status') == 'ready_for_delivery' ? 'selected' : '' }}>
                    {{ __('messages.ready_for_immediate_delivery') }}
                </option>
                <option value="made_to_order" {{ old('product_status') == 'made_to_order' ? 'selected' : '' }}>
