...
<div x-show="showOTP"
    ...
    <div style="background-image: url('{{ asset('images/4d2a165c129977b25a433b916ddfa33f089dcf9f.jpg') }}');"
         class="relative w-[50%] h-auto bg-cover bg-center flex flex-col justify-center items-center rounded-lg shadow-lg mt-[60px]">
        ...
        <div class="relative bg-white w-[588px] h-[566px] p-[60px] rounded-[12px] shadow-xl overflow-y-auto">
            ...
            @foreach (['status', 'success', 'info'] as $msg)
                @if(session($msg))
                    <div class="bg-{{ $msg === 'info' ? 'blue' : 'green' }}-100 border border-{{ $msg === 'info' ? 'blue' : 'green' }}-400 text-{{ $msg === 'info' ? 'blue' : 'green' }}-700 px-4 py-3 rounded relative mb-4" role="alert">
                        {{ session($msg) }}
                    </div>
                @endif
            @endforeach
            ...
            <form action="{{ route('otp.verify.submit') }}" method="POST" id="otp-form">
                ...
                <div class="otp-input-container flex justify-center gap-2" dir="ltr">
                    @for($i = 1; $i <= 4; $i++)
                        <input type="text" id="otp{{ $i }}" name="otp_digit[]" class="otp-input w-12 h-12 text-center border rounded text-xl"
                               maxlength="1" pattern="[0-9]" inputmode="numeric" autocomplete="one-time-code" required>
                    @endfor
                    ...
                    @if(session('identifier_type') === 'email')
                       {{__('messages.use_phone_instead')}}
                        <form action="{{ route('otp.switch.method') }}" method="POST" class="inline-block">
                            ...
                            <button type="submit" class="text-blue-600 hover:underline">(رقم الهاتف)</button>
                        </form>
                    @else
{{__('messages.use_email_instead')}}                        <form action="{{ route('otp.switch.method') }}" method="POST" class="inline-block">
                            @csrf
                            <input type="hidden" name="method" value="email">
                            <button type="submit" class="text-blue-600 hover:underline">(البريد الإلكتروني)</button>
                        </form>
                    @endif
                </p>

                <button type="submit" id="verify-button"
                        class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg mt-6"
                        disabled>
                    {{__('messages.confirm')}}
                </button>
            </form>
