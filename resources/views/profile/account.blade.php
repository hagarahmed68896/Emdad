...
@section('content')
    ...
    <div class="min-h-screen flex flex-col items-center py-8">
        <div class="w-full max-w-7xl bg-white rounded-xl shadow-lg flex flex-col md:flex-row overflow-hidden">
            ...
            <main class="w-full md:w-3/4 p-6 bg-white">
                <div class="mb-6">
                    <h1 class="text-3xl font-bold text-gray-800 mb-2">{{__('messages.MyAccount')}}</h1>
                    <p class="text-sm text-gray-500">{{__('messages.home')}} <span class="mx-1">&gt;</span> {{__('messages.MyAccount')}}</p>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="last_name" name="last_name"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 transition-all duration-200 outline-none"
                                    placeholder="{{__('messages.enter_family_name')}}" value="{{ $user->last_name }}">
                                @error('last_name')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="phone_number" name="phone_number"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 transition-all duration-200 outline-none"
                                    placeholder=" {{__('messages.phoneMSG')}}" value="{{ $user->phone_number }}">
                                @error('phone_number')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="address" name="address"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-green-500 focus:border-green-500 transition-all duration-200 outline-none"
                                    placeholder="{{__('messages.addressMSG')}}" value="{{ $user->address }}">
                                @error('address')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                <section class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                    <h2 class="text-2xl font-semibold text-gray-800 mb-6"> {{__('messages.password')}}</h2>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div class="md:col-span-2">
                                <label for="password_confirmation"
                                    class="block text-gray-700 text-sm font-medium mb-2">{{__('messages.passwordConfirm')}}</label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const openProfileModalBtn = document.getElementById(
            'openProfileModalBtn'); // Pen icon button to open modal
            ...
            'removePhotoBtn'); // "Remove Photo" button inside modal (can be null if no picture)
            ...
            openProfileModalBtn.addEventListener('click', function() {
                ...
                .src; // Ensure modal shows the current main profile image
            ...
            profilePictureInput.addEventListener('change', function(event) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        console.log('FileReader loaded. Updating profilePageImage.src to:', e.target
                            .result);
                        profileImage.src = e.target
                        .result; // Display a real-time preview on the main page
                    ...
                    file); // Read the file as a data URL (this triggers reader.onload asynchronously)
                } else {
                    console.log(
                    'No file selected in input. Clearing selected file and hiding save button.');
            ...
            saveProfilePhotoBtn.addEventListener('click', async function() {
                if (!selectedFile) {
                    ...
                    'الرجاء تحديد صورة لتحميلها.'; // Please select an image to upload.
                    console.log('Save button clicked but no file selected.');
                    return;
                }

                uploadLoading.classList.remove('hidden'); // Show the loading indicator
                saveProfilePhotoBtn.classList.add(
                'hidden'); // Hide the "Save Photo" button during upload
                ...
                '{{ csrf_token() }}'); // Append the CSRF token for Laravel security
                ...
                try {
                    ...
                    if (response.ok) { // If the HTTP status code is 2xx (success)
                        ...
                        profileImage.src =
                        newImageUrl; // Update the main image with the new URL from the server
                        ...
                        'تم تحديث صورة الملف الشخصي بنجاح!'; // Display success message
                        ...
                        if (data.message) {
                            ...
                            errorMessage = data.errors.profile_picture[
                            0]; // Display specific validation error
                        }
                        uploadMessage.className = 'mt-2 text-sm text-red-500 text-center';
                        uploadMessage.textContent = errorMessage;
                        saveProfilePhotoBtn.classList.remove(
                        'hidden'); // Show the "Save Photo" button again for retry
                    ...
                    uploadMessage.className = 'mt-2 text-sm text-red-500 text-center';
                    uploadMessage.textContent =
                    'حدث خطأ غير متوقع. حاول مرة أخرى.'; // Display unexpected error message
                }
            });
                ...
                removePhotoBtn.addEventListener('click', async function() {
                    ...
                    try {
                        ...
                        if (response.ok) { // If successful
                            ...
                            const placeholderUrlWithCacheBuster =
                                '{{ asset('images/Unknown_person.jpg') }}' + '?' + new Date()
                            .getTime();
                            profileImage.src =
                            placeholderUrlWithCacheBuster; // Set main image to your specified placeholder
                            modalProfileImage.src =
                            placeholderUrlWithCacheBuster; // Set modal image to your specified placeholder

                            uploadMessage.className = 'mt-2 text-sm text-green-500 text-center';
                            uploadMessage.textContent = data.message ||
                                'تمت إزالة صورة الملف الشخصي بنجاح.'; // Display success message
                            saveProfilePhotoBtn.classList.add(
                            'hidden'); // Hide the "Save Photo" button (no new image selected)
                            if (removePhotoBtn) { // Hide remove button in modal if it was present
                                removePhotoBtn.classList.add('hidden');
                            }
                            console.log('Profile picture removed successfully. Displaying placeholder:',
                                placeholderUrlWithCacheBuster);
